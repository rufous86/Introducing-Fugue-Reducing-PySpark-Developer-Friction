...
(*Все фото взяты из оригинальной статьи*)
...
В приведенном ниже коде мы при разбиении данных отбираем указанное количество одинаковых значений по столбцу `col`. Предварительная сортировка применяется по мере разбиения данных по столбцу `col2`.
...
dag = FugueWorkflow()
df = dag.df(data)
df = df.partition_by("col", presort="col2 desc", algo="rand").take(3)
dag.run(spark_session)
df.result.show()
```
